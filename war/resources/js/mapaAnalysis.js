...
$(document).on("pageshow","#mapa-analysis-page",function() {
...
$(document).on('tap','[action="consultarAnalysis"]',function(event) {
					...
					if (fecha_desde == "" || fecha_hasta == "") {
						...
						if (f_desde==true && f_hasta==true){
							...
							if (d > h) {
									...
									if (valorflip == 'on') {
										...
										if (clase == "2" || clase == "3") {
											...
											if (clase == "1") {
												...
												var basicSliderMin = $("#" + id).rangeSlider("min");
												var basicSliderMax = $("#" + id).rangeSlider("max");
												valor.idvariable = id;
												valor.tipo = clase;
												valor.valormin = Math.round(basicSliderMin);
												valor.valormax = Math.round(basicSliderMax);
												...
												if (clase == "4") {
													...
													valor.idvariable = id;
													valor.tipo = clase;
													valor.valormin = Math.floor(basicSliderMin * 10) / 10;
													valor.valormax = Math.floor(basicSliderMax * 10) / 10;
								...
								//setTimeout('mostrarPuntos(fecha_desde,fecha_hasta, json);',1500);
...
//dibujo el cluster
$(document).on('tap', '[action="clusterAnalysis"]', function(event) {
...
//dibujo el heatmap
$(document).on('tap', '[action="heatmapAnalysis"]', function(event) {
...
$(document).on('tap', '[action="delete_points_analysis"]', function(event) {
	$(this).simpledialog(
			{
				'mode' : 'bool',
...
$(document).on('tap', '[action="store_location_analysis"]', function(event) {
...
function mostrarPuntos (fecha_desde,fecha_hasta, json)
{
	...
	$.mobile.showPageLoadingMsg("a", "Cargando datos...");
...
$(document).on('tap','[action="visualizarUsuarios"]',function(event) {
	var usersIdentifiers = $("#usersArea").val();
	$('#mapa_num_usuarios').html('2');
	...
	if (fecha_desde == "" || fecha_hasta == "") {
		...
		if (f_desde==true && f_hasta==true){
			...
			if (d > h) {
				...
				if(usersIdentifiers==''){
					$('#filtro-error').html('<font color="#FF0000">Debe introducir al menos un identificador de usuario</font>');
				}else{
					$('#filtro-error').html('');
					...
					if (fijado)  $('#id_comparar').removeClass('ui-disabled');
					mostrarPuntosUsuarios(fecha_desde,fecha_hasta, usersIdentifiers);				
					//mostrarUsuarios(fecha_desde,fecha_hasta, json);	
				}
...
function mostrarPuntosUsuarios (fecha_desde,fecha_hasta, usersID)
{
	var basicHourValues = $("#hour_slider").rangeSlider("values");
	
	$.mobile.showPageLoadingMsg("a", "Cargando datos...");
	$.ajax({type : "POST",url : domain()
		+ "/api/puntos/usersLocation?fecha_desde='"
		+ fecha_desde
		+ "'&fecha_hasta='"
		+ fecha_hasta
		+ "'&horaMin="
